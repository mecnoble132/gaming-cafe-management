...
test.describe('Phase 4: Cookie Consent & Error Boundary E2E Tests', () => {
  ...
  test('Cookie Consent Banner appears, redirects, and accepts correctly', async ({ page }) => {
    ...
    const banner = page.locator('text=Cookie & Storage Notice').locator('../../../..');
    const bannerText = page.locator('text=Cookie & Storage Notice');
    await expect(bannerText).toBeVisible({ timeout: 5000 });

    // 2. Verify links to Privacy Policy and Terms of Service are present
    // Scope to the banner container to avoid strict-mode collision with footer buttons
    const privacyBtn = banner.getByRole('button', { name: 'Privacy Policy' });
    const termsBtn = banner.getByRole('button', { name: 'Terms of Service' });
