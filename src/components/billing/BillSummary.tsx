...
export function BillSummary({ 
  ...
  const sessionMinutes = items
    .filter((item) => item.item_type === 'session')
    .reduce((acc, item) => acc + Number(item.metadata?.duration_minutes ?? 0), 0);
  const earnablePoints = Math.floor(sessionMinutes / loyaltySettings.earn_rate_minutes) * loyaltySettings.earn_rate_points;

  const redeemableSessions = items.filter(
    (item) => item.item_type === 'session'
  );
