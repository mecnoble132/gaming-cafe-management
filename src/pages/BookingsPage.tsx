...
export default function BookingsPage({
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [
        ...
        supabase.from('booking_settings').select('opening_time,closing_time,slot_minutes').maybeSingle(),
  ...
  const saveDraft = async () => {
    ...
    if (draft.kind === 'block') {
      const next: Partial<BookingOrBlock> = {
        kind: 'block',
        station_id: draft.station_id,
      ...
      if (activeId) next.id = activeId;

      const { data: saved, error } = await supabase.from('bookings').upsert(next).select().single();
      if (error) {
      ...
      setItems((prev) => {
        ...
        return [...rest, saved as BookingOrBlock].sort(
                              ...
                              return (
                                <button
                                  ...
                                  className={cn(
                                    ...
                                    type ? DEFAULT_TYPE_COLOR : 'bg-muted/30 border-border text-muted-foreground',
